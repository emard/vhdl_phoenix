...
$(project).svf: $(project).bit
	cp $< default.bit
	$(xil_env); impact -batch programmer/bit2svf.ut
	mv default.svf $@
	rm default.bit
...
$(project).xsvf: $(project).bit
	cp $< default.bit
	$(xil_env); impact -batch programmer/bit2xsvf.ut
	mv default.xsvf $@
	rm default.bit
junk += $(project).xsvf
...
program: $(project).svf
	openocd --file=$(openocd_interface) --file=programmer/$(project).ocd
...
junk += iseconfig _ngo webtalk_pn.xml
